...
**Status:** All recommendations accepted or strengthened; four human decisions
remain open; Phase 1 is not authorized.
...
- `REF-OPEN-1` through `REF-OPEN-4` require the human's explicit decision.
...
| REF-REC-4 | Accepted with stronger minimum; human policy pending | The recommended cutover is rotation/revocation of legacy session and invitation credentials plus explicit re-login. Retained archives are encrypted, access-controlled, logged, and time-bounded regardless because they contain other sensitive data. `REF-OPEN-2` records the human-facing cutover decision. Phase 2 must prove no archived credential authenticates. |
| REF-REC-5 | Accepted with provider-constrained wording | ADR-006 defines a server-only App private key, just-in-time single-repository installation tokens, explicit permissions, operation-bound/single-consumption Norns brokering, ephemeral askpass use, post-operation GitHub revocation, audit, and redaction. GitHub sets installation-token expiry at one hour, so Norns does not falsely promise a minutes-only cryptographic lifetime; it mints immediately before one bounded operation and proves replay rejection after revocation. Landing is before Phase 3 GitHub work if `REF-OPEN-1` keeps it in MVP. |
...
| REF-REC-9 | Accepted | The program now defines per-phase/per-role FSE accounting, a 9–16 week planning window, 116 planned FSE, 29 contingency, a proposed 145-FSE ceiling, per-gate variance reporting, defined 100%/125%/150% controls with automatic pause at 150%, and the required tabletop pause test. A proposed seat-first $500 incremental API hard cap awaits `REF-OPEN-4`. |
...
|---|---|---|
| REF-OPEN-1 — GitHub App in MVP | **Include it.** The user explicitly requested GitHub login and repository selection. Use local binding for the first runner vertical slice, but complete GitHub App binding before the real-project pilot. | Pending human decision. Excluding it reduces the forecast by 4 FSE and requires an explicit MVP/gate scope change. |
| REF-OPEN-2 — session cutover | **Approve mandatory rotation/revocation of every active session and unused invitation at cutover, require one explicit re-login, and encrypt/restrict retained archives.** | Pending human approval. Rotation removes replay risk; archive controls protect the remaining sensitive data. |
| REF-OPEN-3 — concurrent active phases | **Default to one executing phase per project for MVP.** Multiple proposed/approved phases may exist; the schema continues to support a later policy increase. | Pending human decision. More than one executing phase adds an estimated 6–10 FSE across Phases 4–6. |
| REF-OPEN-4 — effort/cost envelope | **Approve the seat-first hybrid:** 116 planned FSE + 29 contingency = 145 maximum; expected incremental API spend $150–300, hard cap $500. | Pending human decision. Balanced hybrid ($1,000) and API-first ($2,500) remain alternatives. |
...
- The human explicitly authorizes the final FSE/API envelope.
...
`REF-REC-5`, if GitHub App remains in MVP.
