...
- [ ] EXEC-WAIT-1 — 🟡 **Human-input request fails the run when the worktree has uncommitted work.** StrumSheetV01 attempt 4 ran 16 minutes of real work (37 paths), then the agent asked a human question and the runner refused: "human input was requested with 37 uncommitted path(s); the runner refused to release an ephemeral worktree that could not be resumed exactly" — run failed and the uncommitted work was lost (worktree not preserved). Options: auto-commit a WIP checkpoint before opening the human wait, or instruct the runtime to commit before asking. As-is, every mid-task question from the coding agent torches its own progress.
- [ ] EXEC-REVIEW-1 — 🔴 **The independent reviewer stage does not exist.** Every succeeded development run parks its task at `in_review` forever: `agent_reviews` has never contained a row, no worker reviews runs, and the UI has no review/approve control. Undiscovered until StrumSheetV01 run 15 became the first production run to reach the stage. Foundation was completed via `Phase4CompletionService` as an explicit operator action (real verification digest + published commit as evidence, summary states the gap). Build either an autonomous reviewer worker (phase 6 already assigns `reviewer_agent_profile_id`) or a UI approve path — until then every phase needs the operator completion.
- [ ] EXEC-CANCEL-2 — 🟡 **Watchdog for delivered-but-never-started dispatch commands.** The stuck command's `expires_at` passed hours before anyone noticed, but expiry is only enforced pre-delivery; a command in `dispatched` status past its envelope `expires_at` should expire the run so the task offers recovery without requiring a user Stop. Second guard from the EXEC-CANCEL-1 incident.
