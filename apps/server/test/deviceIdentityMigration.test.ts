...
describe.sequential("device identity core migration", () => {
  ...
  it("is registered before the additive Phase 2 device migrations", async () => {
    ...
    expect(sources.slice(-21).map((source) => source.name)).toEqual([
      DEVICE_IDENTITY_CORE_MIGRATION_NAME,
      DEVICE_HTTP_REQUEST_REPLAYS_MIGRATION_NAME,
      DEVICE_CANCELLATION_TRACKING_MIGRATION_NAME,
      GATEWAY_DEVICE_AUTHORIZATION_MIGRATION_NAME,
    ...
    expect(sources.at(-21)?.sql).toContain("CREATE TABLE IF NOT EXISTS devices");
    expect(sources.at(-17)?.sql).toContain("ADD COLUMN os_version TEXT");
    expect(sources.at(-16)?.sql).toContain("CREATE TABLE IF NOT EXISTS device_publication_permits");
    expect(sources.at(-15)?.sql).toContain("ADD COLUMN idempotency_key TEXT");
    expect(sources.at(-14)?.sql).toContain("CREATE TABLE legacy_repository_binding_claims");
    expect(sources.at(-13)?.sql).toContain("norns.runner-http.repository-registration.v1");
    expect(sources.at(-12)?.sql).toContain("ADD COLUMN markdown_artifacts JSONB");
    expect(sources.at(-11)?.sql).toContain("ADD CONSTRAINT attachments_mime_check");
    expect(sources.at(-10)?.sql).toContain("ADD COLUMN paused_checkpoint TEXT");
    expect(sources.at(-9)?.sql).toContain("status = 'awaiting_human'");
    expect(sources.at(-8)?.sql).toContain("ADD COLUMN resume_idempotency_key TEXT");
    expect(sources.at(-7)?.sql).toContain("ADD COLUMN adjudications JSONB");
    expect(sources.at(-6)?.sql).toContain("parked at Gate A awaiting a human decision");
    expect(sources.at(-5)?.sql).toContain("conversation_plan_reviews_awaiting_human_idx");
    ...
    expect(sources.at(-3)?.sql).toContain("default_max_rounds BETWEEN 0 AND 5");
    expect(sources.at(-2)?.sql).toContain("ADD COLUMN last_human_message_at TIMESTAMPTZ");
    expect(sources.at(-1)?.sql).toContain("ADD COLUMN qc_mode_changed_at_round INTEGER");
  });
