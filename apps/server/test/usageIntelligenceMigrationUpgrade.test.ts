...
describe.sequential("usage intelligence migration upgrade path", () => {
  ...
  beforeAll(async () => {
    ...
    upgrades = await migrationRange(28, 34);
  ...
  it("applies 0028 through 0034 to legacy data without losing attribution", async () => {
    ...
    expect(applied.map((result) => [result.name, result.applied])).toEqual([
      ["0028_codex_reasoning_effort", true],
      ["0029_global_rules", true],
      ["0030_ai_usage_telemetry", true],
      ["0031_project_access_attribution", true],
      ["0032_usage_intelligence_policies", true],
      ["0033_usage_calibration_analytics", true],
      ["0034_shadow_evidence_order", true],
    ]);
